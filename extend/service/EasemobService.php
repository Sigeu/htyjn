...
class EasemobService{
	...
	public function __construct($options) {
		...
		// 设置环信数据存储到文件的文件基本路径
        $this->basePath = '/static/upload/easemob';
	}
	/**
	* 获取token
	*/
	...
	*/
	...
	function writeCursor($filename,$content){
		//判断文件夹是否存在，不存在的话创建
		if(!file_exists($this->basePath."txtfile")){
			mkdirs($this->basePath."txtfile");
		}
		$myfile=@fopen($this->basePath."txtfile/".$filename,"w+") or die("Unable to open file!");
	...
	function readCursor($filename){
		...
		$file=$this->basePath."txtfile/".$filename;
	...
	*/
	function downloadFile($uuid,$shareSecret){
		...
		if(!file_exists($this->basePath."down")){
			//mkdir("../image/down");
			mkdirs($this->basePath."down/");
		}
		
		$file = @fopen($this->basePath."down/".$filename,"w+");//打开文件准备写入
	...
	*/
    ...
    /*
        群组添加管理员
    */
    function addGroupManager($group_id,$options){
        $url=$this->url.'chatgroups/'.$group_id.'/admin/'.$options;
        $header=array($this->getToken(),'Content-Type:application/json');
        $result=$this->postCurl($url,'',$header);
        return $result;
    }
    /*
		群组移除管理员
	*/
    function deleteGroupManager($group_id,$oldAdmin){
     ...
     * 群组禁言用户
     */
    function addForbiddenTalk($group_id,$options){
        $url=$this->url.'chatgroups/'.$group_id.'/mute/'.$options;
        $header=array($this->getToken(),'Content-Type:application/json');
        $result=$this->postCurl($url,'',$header);
        return $result;
    }
     ...
     * 群组取消禁言用户
     */
    function deleteForbiddenTalk($group_id,$username){
