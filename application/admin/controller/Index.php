...
//            $this->error('系统超级账号禁止操作！'); // todo
        ...
        if ($this->request->isGet()) {
            ...
            if (admin_md5($data['oldpassword']) !== $user['password']) {
                $this->error('旧密码验证失败，请重新输入！');
            }
            if (DataService::save('SystemUser', ['id' => session('user.id'), 'password' => admin_md5($data['password'])])) {
