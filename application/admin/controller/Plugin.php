...
//            dump($config);die;
            ...
            $row !== false ? $this->success('恭喜，保存成功哦！', url('/').'admin.html#'.url('index').'?spm='.$this->spm) : $this->error('保存失败，请稍候再试！');

        }

        $this->assign('plugin',$row);
        $this->assign('title', '插件管理 - '.$row['name'].'配置');
