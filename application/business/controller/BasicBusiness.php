 ...
 */
class BasicBusiness extends Controller {
    ...
    public function _initialize() {
        ...
        $sjUid = $this->request->param('uid', 0);
        list($module, $controller, $action) = [$this->request->module(), $this->request->controller(), $this->request->action()];

        if ($adminUser['authorize'] == 3 && $sjUid) {
            // 系统管理员 有权限查看公墓系统信息
            $uid = $this->request->param('uid');
            $user = Db::name('business_staff')->where('create_by', $uid)->find();
            ...
            // 用户登录状态检查
            if (($this->checkLogin || $this->checkAuth) && !session('b_user')) {
                $this->redirect('@business/login');
            }

            // 节点访问权限检查
            if ($this->checkLogin && $this->checkAuth && !b_auth("{$module}/{$controller}/{$action}")) {
