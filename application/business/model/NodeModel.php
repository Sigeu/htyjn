...
class NodeModel {
    ...
    public static function applyAuthNode() {
        cache('b_need_access_node', null);
        if (($authorize = session('b_user.authorize'))) {
            $authorizeids = Db::name('BusinessAuth')->where('id', 'in', explode(',', $authorize))->where('status', '1')->column('id');
            if (empty($authorizeids)) {
                return session('b_user.nodes', []);
            }
            $nodes = Db::name('BusinessAuthNode')->where('auth', 'in', $authorizeids)->column('node');
            return session('b_user.nodes', $nodes);
        }
        return false;
    }
    ...
    public static function getAuthNode() {
        static $nodes = [];
        if (empty($nodes)) {
            $nodes = cache('b_need_access_node');
            if (empty($nodes)) {
                $nodes = Db::name('BusinessNode')->where('is_auth', '1')->column('node');
                cache('b_need_access_node', $nodes);
            }
        }
        return $nodes;
    }
    ...
    public static function checkAuthNode($node) {
        ...
        if (session('b_user.username') === 'admin' || stripos($node, 'admin/index') === 0) {
            return true;
        }
        if (!in_array($auth_node, self::getAuthNode())) {
            return true;
        }
        return in_array($auth_node, (array) session('b_user.nodes'));
    }
    ...
    static public function get() {
        $alias = [];
        foreach (Db::name('BusinessNode')->select() as $vo) {
            $alias["{$vo['node']}"] = $vo;
        }
        $nodes = [];
        $ignore = [
     ...
     * 获取公墓模块节点
     */
    static public function getNeedle($needle = []) {
        $alias = [];
        foreach (Db::name('BusinessNode')->select() as $vo) {
            $alias["{$vo['node']}"] = $vo;
        }
