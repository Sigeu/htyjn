...
function update_pay_status($order_sn, $ext=array())
{
    ...
    $orderInfo = M('recharge')->where('pd_sn', $order_sn)->find();
    if ($orderInfo['pd_payment_state'] == 1) {
        // 已经收到过异步通知 不允许重复更改支付状态
        return ;
    }else {
        // 修改支付状态  已支付
        $update = array('pd_payment_state'=>1, 'pd_order_state' => 4, 'pd_payment_time' => time());
        ...
        // 将纪念币充值进用户账号
        \app\api\model\MemberModel::self()->rechareCoins($orderInfo['pd_member_id'], $orderInfo['pd_coins']);
        M('recharge')->where('pd_sn', $order_sn)->update($update);
    }
 ...
 * @param $order_state
 */
function update_order_status($order_sn, $order_state)
{
    // 修改订单状态
    $update = ['pd_order_state' => $order_state];
    M('recharge')->where('pd_sn', $order_sn)->update($update);
}
